...
from diffpep.models_con.edge import EdgeEmbedder
from diffpep.models_con.node import NodeEmbedder
from diffpep.modules.common.layers import sample_from, clampped_one_hot, sample_from_contact
from diffpep.models_con.ga import GAEncoder
from diffpep.modules.protein.constants import AA, BBHeavyAtom, max_num_heavyatoms
from diffpep.modules.common.geometry import construct_3d_basis
from diffpep.utils.data import mask_select_data, find_longest_true_segment, PaddingCollate
from diffpep.utils.misc import seed_all
from diffpep.utils.train import sum_weighted_losses
from torch.nn.utils import clip_grad_norm_

from diffpep.modules.so3.dist import centered_gaussian,uniform_so3
from diffpep.modules.common.geometry import batch_align, align
...
from diffpep.utils.misc import load_config
from diffpep.utils.train import recursive_to
from easydict import EasyDict

from diffpep.models_con.utils import process_dic
from diffpep.models_con.torsion import get_torsion_angle, torsions_mask
import diffpep.models_con.torus as torus
##### 
from diffpep.modules.interaction.parsers import contact_map_complex, create_unified_mask, remove_self_contacts
...
class FlowModel(nn.Module):
    def __init__(self, cfg):
        ...
        self.sample_contact = self._interpolant_cfg.sample_contact

        self.K = self._interpolant_cfg.seqs.num_classes
        self.k = self._interpolant_cfg.seqs.simplex_value
        self.mode = self._interpolant_cfg.mode
        self.threshold = self._interpolant_cfg.threshold
    ...
    def encode(self, batch):
        ...
        contact_1 = contact_map_complex(batch, mode=self.mode, threshold=self.threshold)
        contact_1 = torch.where(batch['res_mask'][:, :, None] * batch['res_mask'][:, None, :], contact_1, 0.)
        ...
        return rotmats_1, trans_1, angles_1, seqs_1, contact_1, node_embed, edge_embed
    ...
    def contact_to_simplex(self, contact):
        return clampped_one_hot(contact.long(), 2).float() * 2 * 2 - 2
    ...
    def forward(self, batch):
        ...
        contact_mask = create_unified_mask(batch, mode="complex")
        
        # encode receptor infomation 
        rotmats_1, trans_1, angles_1, seqs_1, contact_1, node_embed, edge_embed = self.encode(batch) # no generate mask
        ...
        contact_1_simplex = self.contact_to_simplex(contact_1)
        contact_1 = contact_1_simplex.argmax(dim=-1)
        ...
        with torch.no_grad():
            ...
            if self.sample_contact:
                contact_0_simplex = 2 * torch.randn_like(contact_1_simplex) # (B,L,L,2)
                contact_0_prob = F.softmax(contact_0_simplex,dim=-1)
                contact_t_simplex = (1 - t[..., None, None]) * contact_0_simplex + t[..., None, None] * contact_1_simplex
                contact_t_simplex = torch.where(contact_mask[...,None], contact_t_simplex, contact_1_simplex)
                contact_t_prob = F.softmax(contact_t_simplex,dim=-1)
                contact_t = sample_from_contact(contact_t_prob) # (B,L,L)
                contact_t = torch.where(contact_mask, contact_t, contact_1)
                contact_t = remove_self_contacts(contact_t)
            else:
                contact_t = contact_1.detach().clone()

        # denoise
        pred_rotmats_1, pred_trans_1, pred_angles_1, pred_seqs_1_prob, pred_contact_1_prob  = self.ga_encoder(t, rotmats_t, trans_t_c, angles_t, seqs_t, contact_t, 
        ...
        # contact aux loss
        contact_mask = create_unified_mask(batch, mode="complex")
        contact_loss = F.cross_entropy(pred_contact_1_prob.view(-1,pred_contact_1_prob.shape[-1]),contact_1.view(-1), reduction='none').view(pred_contact_1_prob.shape[:-1]) # (N,L,L), not softmax
        contact_loss = torch.sum(contact_loss * contact_mask, dim=(-1,-2)) / (torch.sum(contact_mask,dim=(-1,-2)) + 1e-8)
    ...
    def sample(self, batch, num_steps = 100, sample_bb=True, sample_ang=True, sample_seq=True,
        ...
        #encode
        rotmats_1, trans_1, angles_1, seqs_1, contact_1, node_embed, edge_embed = self.encode(batch)
        ...
        contact_1_simplex = self.contact_to_simplex(contact_1)
        contact_1_prob = F.softmax(contact_1_simplex,dim=-1)
        ...
        if sample_contact:
            contact_0_simplex = 2 * torch.randn((num_batch,num_res,num_res,2), device=batch['aa'].device)
            contact_0_prob = F.softmax(contact_0_simplex,dim=-1)
            contact_0 = sample_from_contact(contact_0_prob) # (B,L,L)
            contact_0 = torch.where(contact_mask, contact_0, contact_1)
            contact_0 = remove_self_contacts(contact_0)
            contact_0_simplex = torch.where(contact_mask[...,None], contact_0_simplex, contact_1_simplex)
        else:
            contact_0 = contact_1.detach().clone()
            contact_0_prob = contact_1_prob.detach().clone()
            contact_0_simplex = contact_1_simplex.detach().clone()
        ...
        contact_t_1, contact_t_1_simplex = contact_0, contact_0_simplex
        
        # denoise loop
        for t_2 in ts[1:]:
            t = torch.ones((num_batch, 1), device=batch['aa'].device) * t_1
            # rots
            pred_rotmats_1, pred_trans_1, pred_angles_1, pred_seqs_1_prob, pred_contact_1_prob = self.ga_encoder(t, rotmats_t_1, trans_t_1_c, angles_t_1, seqs_t_1, contact_t_1,
                                                                                            node_embed, edge_embed, batch['generate_mask'].long(), batch['res_mask'].long())
            ...
            # contact 
            pred_contact_1 = sample_from_contact(F.softmax(pred_contact_1_prob,dim=-1)) # (B,L,L)
            pred_contact_1 = torch.where(contact_mask, pred_contact_1, contact_1)
            pred_contact_1 = remove_self_contacts((pred_contact_1>0.5).float())
            pred_contact_1_simplex = self.contact_to_simplex(pred_contact_1)
            ...
            if not sample_contact:
                pred_contact_1 = contact_1.detach().clone()
                pred_contact_1_simplex = contact_1_simplex.detach().clone()
            clean_traj.append({'rotmats':pred_rotmats_1.cpu(),'trans':pred_trans_1_c.cpu(),
                               ...
                               'contact':pred_contact_1.cpu(),
                               'contact_simplex':pred_contact_1_simplex.cpu(),
                               # 真实值
                                'rotmats_1':rotmats_1.cpu(),'trans_1':trans_1_c.cpu(),
                                'angles_1':angles_1.cpu(),'seqs_1':seqs_1.cpu(), 
                                'contact_1':contact_1.cpu()})
            ...
            # contact 
            contact_t_2_simplex = contact_t_1_simplex + (pred_contact_1_simplex - contact_0_simplex) * d_t[...,None,None]
            contact_t_2 = sample_from_contact(F.softmax(contact_t_2_simplex,dim=-1)) # (B,L,L)
            contact_t_2 = torch.where(contact_mask, contact_t_2, contact_1)
            contact_t_2 = remove_self_contacts(contact_t_2)
            ...
            if not sample_contact:
                contact_t_2 = contact_1.detach().clone()
            ...
            contact_t_1_simplex, contact_t_1 = contact_t_2_simplex, contact_t_2
            t_1 = t_2

        # final step
        t_1 = ts[-1]
        t = torch.ones((num_batch, 1), device=batch['aa'].device) * t_1
        pred_rotmats_1, pred_trans_1, pred_angles_1, pred_seqs_1_prob, contact_t_1_prob = self.ga_encoder(t, rotmats_t_1, trans_t_1_c, angles_t_1, seqs_t_1, contact_t_1, 
                                                                                                     node_embed, edge_embed, batch['generate_mask'].long(), batch['res_mask'].long())
        pred_rotmats_1 = torch.where(batch['generate_mask'][...,None,None],pred_rotmats_1,rotmats_1)
        # move center
        # pred_trans_1_c,center = self.zero_center_part(pred_trans_1,gen_mask,res_mask)
        ...
        pred_angles_1 = torch.where(torsion_mask.bool(),pred_angles_1,torch.zeros_like(pred_angles_1))
        # contact
        pred_contact_1 = sample_from_contact(F.softmax(contact_t_1_prob,dim=-1)) # (B,L,L)
        pred_contact_1 = torch.where(contact_mask, pred_contact_1, contact_1)
        pred_contact_1 = remove_self_contacts(pred_contact_1)
        pred_contact_1_simplex = self.contact_to_simplex(pred_contact_1)
        ...
        if not sample_seq:
            ...
            pred_seqs_1_simplex = seqs_1_simplex.detach().clone()
        if not sample_contact:
            pred_contact_t_1 = contact_1.detach().clone()
            pred_contact_1_simplex = contact_1_simplex.detach().clone()
            
        clean_traj.append({'rotmats':pred_rotmats_1.cpu(),'trans':pred_trans_1_c.cpu(),
                           'angles':pred_angles_1.cpu(),'seqs':pred_seqs_1.cpu(),
                           'seqs_simplex':pred_seqs_1_simplex.cpu(),
                           'contact':pred_contact_t_1.cpu(),
                           'contact_simplex':pred_contact_1_simplex.cpu(),
                            'rotmats_1':rotmats_1.cpu(),'trans_1':trans_1_c.cpu(),
                            'angles_1':angles_1.cpu(),'seqs_1':seqs_1.cpu(), 
                            'contact_1':contact_1.cpu()})
