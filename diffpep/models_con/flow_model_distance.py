...
from diffpep.modules.common.layers import sample_from, clampped_one_hot, sample_from_distance
from diffpep.models_con.ga import GAEncoderDist, GAEncoderDist_Raw
...
##### 
from diffpep.modules.interaction.parsers import distance_map_complex, create_unified_mask
...
class FlowModel_Raw(nn.Module):
    def __init__(self, cfg):
        super().__init__()
        self._model_cfg = cfg.encoder
        self._interpolant_cfg = cfg.interpolant
        
        self.distance_to_bins = DistanceToBins(dist_min=self._interpolant_cfg.dist.dist_min, 
                                               dist_max=self._interpolant_cfg.dist.dist_max, 
                                               num_bins=self._interpolant_cfg.dist.num_bins, 
                                               use_onehot=self._interpolant_cfg.dist.use_onehot)
    ...
    def encode(self, batch):
        ...
        dist_1 = distance_map_complex(batch, mode=self.mode)  # (B, L, L)
        dist_1 = torch.where(batch['res_mask'][:, :, None] * batch['res_mask'][:, None, :], dist_1, 0.)
        ...
        return rotmats_1, trans_1, angles_1, seqs_1, dist_1, node_embed, edge_embed
    ...
    def dist_to_simplex(self, dist):
        dist_one_hot = self.distance_to_bins(dist.unsqueeze(-1), dim=-1) 
        return dist_one_hot * self.num_bins * 2 - self.num_bins  # (N, *, num_bins, *)
    ...
    def forward(self, batch):
        ...
        rotmats_1, trans_1, angles_1, seqs_1, dist_1, node_embed, edge_embed = self.encode(batch) # no generate mask
        ...
        dist_1_simplex = self.dist_to_simplex(dist_1)
        dist_1_prob = F.softmax(dist_1_simplex,dim=-1)
        dist_1 = dist_1_simplex.argmax(dim=-1)
        ...
        with torch.no_grad():
            ...
            if self.sample_dist:
                dist_0 = self.num_bins * torch.randn_like(dist_1_simplex) # (B,L,L,num_bins)
                dist_0_prob = F.softmax(dist_0,dim=-1)
                dist_t_simplex = ((1 - t[..., None, None]) * dist_0) + (t[..., None, None] * dist_1_simplex) # (B,L,L,num_bins)
                dist_mask = create_unified_mask(batch, mode="complex")
                dist_t_simplex = torch.where(dist_mask[...,None], dist_t_simplex, dist_1_simplex)
                dist_t_prob = F.softmax(dist_t_simplex,dim=-1)
                dist_t = sample_from_distance(dist_t_prob) # (B,L,L)
                dist_t = torch.where(dist_mask, dist_t, dist_1)
            else:
                dist_t = dist_1.detach().clone()

        # denoise
        pred_rotmats_1, pred_trans_1, pred_angles_1, pred_seqs_1_prob, pred_dist_1_prob  = self.ga_encoder(t, rotmats_t, trans_t_c, angles_t, seqs_t, dist_t, 
        ...
        # contact aux loss
        dist_mask = create_unified_mask(batch, mode="complex")
        dist_loss = F.cross_entropy(pred_dist_1_prob.view(-1,pred_dist_1_prob.shape[-1]),torch.clamp(dist_1,0,self.num_bins-1).view(-1), reduction='none').view(pred_dist_1_prob.shape[:-1]) # (N*L*L), not softmax
        dist_loss = torch.sum(dist_loss * dist_mask, dim=(-1,-2)) / (torch.sum(dist_mask,dim=(-1,-2)) + 1e-8)
    ...
    @torch.no_grad()
    def sample(self, batch, num_steps = 100, sample_bb=True, sample_ang=True, sample_seq=True, sample_dist=True):
        ...
        #encode
        rotmats_1, trans_1, angles_1, seqs_1, dist_1, node_embed, edge_embed = self.encode(batch)
        ...
        if sample_dist:
            dist_0_simplex = self.num_bins * torch.randn((num_batch,num_res,num_res,num_bins), device=batch['aa'].device)
            dist_0_prob = F.softmax(dist_0_simplex,dim=-1)
            dist_0 = sample_from_distance(dist_0_prob) # (B,L,L)
            dist_mask = create_unified_mask(batch, mode="complex")
            dist_0 = torch.where(dist_mask, dist_0, dist_1)
            dist_0_simplex = torch.where(dist_mask[...,None], dist_0_simplex, dist_1_simplex)
        else:
            dist_0 = dist_1.detach().clone()
            dist_0_prob = dist_1_prob.detach().clone()
            dist_0_simplex = dist_1_simplex.detach().clone()
        ...
        dist_t_1, dist_t_1_simplex = dist_0, dist_0_simplex
        
        # denoise loop
        for t_2 in ts[1:]:
            t = torch.ones((num_batch, 1), device=batch['aa'].device) * t_1
            # rots
            pred_rotmats_1, pred_trans_1, pred_angles_1, pred_seqs_1_prob, pred_dist_1_prob = self.ga_encoder(t, rotmats_t_1, trans_t_1_c, angles_t_1, seqs_t_1, dist_t_1,
            ...
            # contact 
            pred_dist_1 = sample_from_distance(F.softmax(pred_dist_1_prob,dim=-1))
            dist_mask = create_unified_mask(batch, mode="complex")
            pred_dist_1 = torch.where(dist_mask, pred_dist_1, dist_1)
            pred_dist_1_simplex = self.dist_to_simplex(pred_dist_1)
            ...
            if not sample_dist:
                pred_dist_1 = dist_1.detach().clone()
                pred_dist_1_simplex = dist_1_simplex.detach().clone()
            ...
            clean_traj.append({'rotmats':pred_rotmats_1.cpu(),'trans':pred_trans_1_c.cpu(),
                               ...
                               'dist':pred_dist_1.cpu(),
                               'dist_simplex':pred_dist_1_simplex.cpu(),
                               # 真实值
            ...
            # contact 
            dist_t_2_simplex = dist_1_simplex + (pred_dist_1_simplex - dist_0_simplex) * d_t[...,None,None]
            dist_t_2 = sample_from_distance(F.softmax(dist_t_2_simplex,dim=-1))
            dist_mask = create_unified_mask(batch, mode="complex")
            dist_t_2 = torch.where(dist_mask, dist_t_2, dist_1)
            ...
            dist_t_1_simplex, dist_t_1 = dist_t_2_simplex, dist_t_2
            t_1 = t_2
...
class FlowModel(nn.Module):
    def __init__(self, cfg):
        ...
        self.node_embedder = NodeEmbedder(cfg.encoder.node_embed_size,max_num_heavyatoms)
        self.edge_embedder = EdgeEmbedder(cfg.encoder.edge_embed_size,max_num_heavyatoms)
        self.ga_encoder = GAEncoderDist(cfg.encoder.ipa)
