...
def contact_one_hot(x):
    mask = (x>=0) & (x<=1) # N,L,L
    x = x.clamp(min=0, max=1)
    y = F.one_hot(x, 2) * mask[...,None]  # N,L,L,2
    return y
...
def sample_from_contact(contact):
    """sample from distance distribution"""
    N,L1,L2,K = contact.size()
    contact = contact.view(N*L1*L2,K) + 1e-8
    x = torch.multinomial(contact,1).view(N,L1,L2)
    return x

def sample_from_distance(dist):
    """sample from distance distribution"""
    N,L1,L2,K = dist.size()
    dist = dist.view(N*L1*L2,K) + 1e-8
    x = torch.multinomial(dist,1).view(N,L1,L2)
    return x

class DistanceToBins(nn.Module):   #这里把 [dist_min, dist_max] 均匀划分成 num_bins 个代表点（中心点），每个距离就归到最近的代表点上。
