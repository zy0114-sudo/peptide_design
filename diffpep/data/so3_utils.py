...
def rotvec_to_rotmat(rotation_vectors: torch.Tensor, tol: float = 1e-7) -> torch.Tensor:
    ...
    rotation_angles = torch.norm(rotation_vectors, dim=-1)  #计算旋转向量的长度 当做角度
...
def geodesic_t(t: float, mat: torch.Tensor, base_mat: torch.Tensor, rot_vf=None) -> torch.Tensor:
    ...
    """
    if rot_vf is None:
        rot_vf = calc_rot_vf(base_mat, mat)  #base_mat 先验分布   #将旋转矩阵转成旋转向量  log
    mat_t = rotvec_to_rotmat(t * rot_vf)  #乘以t  将旋转向量转成旋转矩阵  exp
